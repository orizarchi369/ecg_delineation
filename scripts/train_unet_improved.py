...
parser.add_argument('--min_delta', type=float, default=1e-4, help='minimum F1 improvement to reset patience')
...
# Quiet / warnings
parser.add_argument('--quiet', action='store_true', help='suppress bars and per-epoch prints')
parser.add_argument('--suppress_warnings', action='store_true', help='ignore FutureWarnings')

# Optional periodic checkpoints
parser.add_argument('--checkpoint_interval', type=int, default=0, help='save checkpoint every N epochs (0=off)')
...
# ----------------------------
# Model (UNet 1D with optional BN/Dropout)
# ----------------------------
...
class UNet1D(nn.Module):
    def __init__(self, in_channels=1, out_channels=4, use_bn=False, p_drop=0.0):
        ...
        self.dec2 = ConvBlock1D(64, 32, use_bn, p_drop)   # 32 up + 32 skip
...
def ensure_csv(path):
    if not os.path.exists(path):
        with open(path, 'w', newline='') as f:
            csv.writer(f).writerow(['epoch','train_loss','val_loss','val_f1','lr'])
...
def train_model(model, loaders, num_epochs, lr, output_dir,
                ...
                live_plot, plot_interval,
                quiet=False, checkpoint_interval=0):
    ...
    for epoch in range(1, num_epochs + 1):
        ...
        model.train()
        running = 0.0
        pbar = tqdm(loaders['train'], desc=f'Epoch {epoch}/{num_epochs}', disable=quiet)
        ...
        if live_plot and (epoch % plot_interval == 0 or epoch == num_epochs):
            plot_curves(train_losses, val_losses, val_f1s, os.path.join(output_dir, 'training_curves_live.png'))
...
if __name__ == '__main__':
    ...
    train_model(
        ...
        quiet=args.quiet, checkpoint_interval=args.checkpoint_interval
    )
